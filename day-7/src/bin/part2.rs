...
impl  CardHand {
    ...
    fn get_hand_type(hand: &str) -> HandType {
        ...
        let mut new_hand = hand.clone().to_string();
        ...
        let max_pair = label_counts.iter().max_by_key(|&(_, v)| v);
        if let Some((key, _)) = max_pair {
           new_hand = new_hand.chars().map(|x| match x {
                'J' => *key,
                c => c,
            }).collect();
        }
        let mut label_counts: HashMap<char, u32> = HashMap::new();
        for label in new_hand.chars() {
            let count = label_counts.entry(label).or_insert(0);
            *count += 1;
        }
